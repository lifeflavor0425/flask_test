"""
    ...
    get 방식은 헤더만 전송 -> 고정크기만 전송 -> 보낼량이 최대치가 전해져 있다 -> 소량만 전송
        - http 헤더만 알면, 패킷(전송데이터)를 가로채서 데이터를 해킹할 수 있다. -> 보안취약
    post 방식은 헤더 전송 -> 바디 전송 -> 대량 전송 가능, 구조를 모르기 때문에 보안에 상대적으로 우수, 암호화 가능 -> 보안에 우수
    - 방식
        - form 전송 : <form> 태그 사용, 화면이 껌벅인다
...
# path
# 전달값을 무한대로 늘려주는 옵션
# http://127.0.0.1:5000/news4/1/2/3/4/5/6/home/동국/10
